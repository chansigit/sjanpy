...
#     legend_pos=(1.35, 0.9), colorbar_pos=(1.35, 0.1), 
#     legend_label_spacing=2.0, 
#     colorbar_width="20%", colorbar_height="30%", # Restored these parameters
#     vmin=None, vmax=None,
...
#     fig = plt.figure(figsize=(len(group_names) * width_per_col + 7, len(gene_names) * height_per_row + 3))
    
#     gs = gridspec.GridSpec(2, 3, width_ratios=[2, 10, 10 * dendro_w], 
#                            height_ratios=[10 * dendro_h, 10], wspace=0.05, hspace=0.05)
...
#     plot_df['gene'] = pd.Categorical(plot_df['gene'], categories=gene_names, ordered=True)
...
#             if row_clusters[gene_names[i]] != row_clusters[gene_names[i+1]]:
#                 ax_main.axhline(i + 0.5, color='black', lw=1.2)
#     if col_km:
#         for i in range(len(group_names)-1):
#             if col_clusters[group_names[i]] != col_clusters[group_names[i+1]]:
#                 ax_main.axvline(i + 0.5, color='black', lw=1.2)

#     # 8. Legends
#     legend_elements = [plt.scatter([], [], c='gray', s=p*dot_scale, label=f'{p}%', lw=0) for p in [25, 50, 75, 100]]
#     legend = ax_main.legend(handles=legend_elements, title="Percent\nExpressed", loc='upper left', 
#                    bbox_to_anchor=legend_pos, frameon=False, labelspacing=legend_label_spacing)
    
#     cax = inset_axes(ax_main, width=colorbar_width, height=colorbar_height, loc='lower left', 
#                      bbox_to_anchor=(*colorbar_pos, 1, 1), bbox_transform=ax_main.transAxes, borderpad=0)
#     cbar = plt.colorbar(sc, cax=cax, label='Z-score' if z_score else 'Mean Exp')
#     if title:
#         plt.suptitle(title, fontsize=16, y=0.98) # y可以根据需要微调，避免重叠
#     if save_path:
#         # 关键：显式包含 colorbar 和 legend
#         plt.savefig(save_path, format='pdf', bbox_inches='tight', 
#                     bbox_extra_artists=[cax, legend], transparent=True)

    
#     return fig, ax_main

# ... 前面的 Module 1 和 Module 2 保持不变 ...

# --- Module 3: Main Function (Fixed Version) ---
def complex_dotplot(
    adata, genes, groupby, 
    z_score=True, cluster_rows=True, cluster_cols=True, use_olo=True,
    manual_gene_order=None, manual_group_order=None,
...
# Dummy Data
# np.random.seed(42)
# test_genes = [f"Gene_{i+1:02d}" for i in range(25)]
# test_clusters = [f"Sample_{i+1}" for i in range(10)]
# test_data = [[c, g, np.random.rand()*3, np.random.rand()*100] for c in test_clusters for g in test_genes]
# df_test = pd.DataFrame(test_data, columns=['Cluster', 'Gene', 'AvgExp', 'Pct'])

# fan_dotplot(df_test)
# #print('final_plot_manual_grid.png')