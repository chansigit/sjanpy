...
# ---------------------------------------------------------------------------
# h5py obs/var readers
# ---------------------------------------------------------------------------

def _decode_stringlike(values):
    """Decode h5py string/object arrays without assuming ASCII."""
    ...
    if arr.ndim == 0:
        return _decode_one(arr.item())

    return np.array([_decode_one(v) for v in arr.tolist()], dtype=object).reshape(arr.shape)

def read_obs_h5py(path):
    """Read obs DataFrame directly via h5py.

    Handles single-category categoricals (brain dataset has a `dataset` column
    with only one category stored as a scalar string, which crashes anndata's
    backed reader).
    """
    obs_dict = {}
    with h5py.File(path, "r") as f:
        ...
        if "__categories" in obs_group:
            ...
            if index_key in obs_group:
                ...
                if enc == "string-array":
                    ...
                    obs_index = idx_ds[:]
                    if hasattr(obs_index, "astype") and obs_index.dtype.kind in ("S", "O"):
                        obs_index = _decode_stringlike(obs_index)
            else:
                obs_index = None
        ...
        if obs_index is not None:
            obs_df.index = obs_index
    return obs_df


def read_var_h5py(path):
    """Read var DataFrame directly via h5py."""
    var_dict = {}
    with h5py.File(path, "r") as f:
        var_group = f["var"]
        index_key = var_group.attrs.get("_index", "_index")
        if index_key in var_group:
            idx_ds = var_group[index_key]
            enc = idx_ds.attrs.get("encoding-type", "")
            if enc == "string-array":
                var_index = idx_ds[:]
            else:
                var_index = idx_ds[:]
            if hasattr(var_index, "dtype") and var_index.dtype.kind in ("S", "O"):
                var_index = _decode_stringlike(var_index)
        else:
            var_index = None
        ...
        for key in var_group.keys():
            ...
            if enc_type == "categorical":
                ...
                if categories.ndim == 0:
                    categories = np.array([str(categories)])
                var_dict[key] = pd.Categorical.from_codes(
                    codes, categories=categories
                )
            elif enc_type == "string-array":
                col_data = dataset[:]
